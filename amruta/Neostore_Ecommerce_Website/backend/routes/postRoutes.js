...
router.post("/adduser", (req, res) => {
  ...
  let ins = new userModel({
    ...
    emailAddress: req.body.emailAdd,
...
router.post("/changepass", (req, res) => {
  ...
  userModel.updateOne(
    { _id: req.body.id },
    { $set: { password: req.body.newPass, cartData: req.body.cartData } },
    function (err, docs) {
      if (err) {
        console.log(err);
      } else {
...
router.post("/color", (req, res) => {
  productModel
    ...
    .populate(["colorId"])
    .then((product) => {
      const pro = product.filter(
        (product) => product.colorId.colorName == req.body.colour
      );
      res.send(pro);
    });
});
...
router.post("/filterdata", (req, res) => {
  ...
  productModel
    ...
    .then((product) => {
      const pro = product.filter(
        (product) => product.categoryId.categoryName == req.body.category
      );
      res.send(pro);
    });
});
  ...
  productModel
    ...
    .then((product) => {
      if (req.body.category == "Sofa") {
        ds = product.filter(
          (product) => product.categoryId.categoryName == "Sofa"
        );
        sofaData = ds.filter((ds) => ds.colorId.colorName == req.body.colour);
        res.send(sofaData);
      } else if (req.body.category == "Bed") {
        ds = product.filter(
          (product) => product.categoryId.categoryName == "Bed"
        );
        sofaData = ds.filter((ds) => ds.colorId.colorName == req.body.colour);
        res.send(sofaData);
      } else if (req.body.category == "Wadrobe") {
        ds = product.filter(
          (product) => product.categoryId.categoryName == "Wadrobe"
        );
        sofaData = ds.filter((ds) => ds.colorId.colorName == req.body.colour);
        res.send(sofaData);
      } else if (req.body.category == "Chair") {
        ds = product.filter(
          (product) => product.categoryId.categoryName == "Chair"
        );
        sofaData = ds.filter((ds) => ds.colorId.colorName == req.body.colour);
        res.send(sofaData);
      } else if (req.body.category == "Dining set") {
        ds = product.filter(
          (product) => product.categoryId.categoryName == "Dining set"
        );
        sofaData = ds.filter((ds) => ds.colorId.colorName == req.body.colour);
        res.send(sofaData);
      }
...
router.put("/updateaddress", (req, res) => {
  console.log(req.body);
  userModel.updateOne(
    { _id: req.body.id, "address.address_id": req.body.address_id },
    { "address.$": req.body },
    (err, result) => {
      if (err) {
        console.log(err);
        ...
        res.json({
          err: 0,
          message: "You Have Successfuly updated Your shipping address.",
...
router.put("/deliveryaddress", (req, res) => {
  ...
  userModel.updateMany(
    { _id: req.body.id },
    { $set: { "address.$[].isDeliveryAddress": false } },
    (err, result) => {
      if (err) throw err;
      else {
        console.log(result);
  ...
  userModel.updateOne(
    { _id: req.body.id, "address.address_id": req.body.addressId },
...
router.put("/updaterating", (req, res) => {
  ...
  productModel.updateOne(
    { _id: req.body.id },
    { $push: { RatingArray: req.body.productratingvalue } },
    function (err, docs) {
      if (err) {
        console.log(err);
        res.json({
        ...
        productModel.updateOne(
          ...
          function (err, docs) {
            if (err) {
              ...
              res.json({
                err: 0,
                message: "You Have Successfuly Updated Your Rating.",
                docs,
              });
            }
          }
